...
* `snippet()` code containing double quotes or `{{`/`}}` patterns now renders correctly. Code is written to a separate file and loaded by Typst rather than being interpolated into the template.
...
* `snippet()` gains a `clip` argument. When `TRUE`, the rendered PNG is copied to the system clipboard immediately after rendering.
